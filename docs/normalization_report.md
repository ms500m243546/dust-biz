...
**Phase S.1 — Logistic-regression source attribution model (2026-05-03).**
Status: complete. New `app/models/attribution/logreg_v0_1_0.py:LogRegAttributor` implements `SourceAttributionModel` per `docs/model-contracts.md`, scoring per-candidate probabilities via a fitted logistic regression over four hand-engineered features (`wind_alignment_score`, `activity_intensity`, `dust_potential_factor`, `concurrent_pm_rise`). Cold-start (no artifact) uses a hand-tuned linear sigmoid at parity with the rules baseline so the model is usable before training. New trainer in `app/training/attribution_training.py` fits a `LogisticRegression` head with self-distilled labels (rank-based split of rules-baseline scores) plus `KFold(n=4)` CV to compute feature-importance variance. Artifact bundles: classifier, feature_columns, feature_importances, feature_importance_cv_std. EvidenceClass assignment is now learned-data-aware (per `docs/causal-protocol.md`): `expert_judgment` for cold-start, `quasi_experimental` when any feature passes both `QE_IMPORTANCE_FLOOR=0.10` and `QE_CV_STD_CEILING=0.02` thresholds, `observational_correlational` otherwise. `experimental` is never assigned (no RCT data). Both `RulesBaselineAttributor` and `LogRegAttributor` are now exported from `app.models.attribution`. 6 new tests covering: kind/version, empty-candidates path, cold-start downwind ranking, fitted-artifact ingestion + evidence-class change, threshold sanity, min-pool enforcement. Risks introduced: self-distilled labels are a stepping stone — S.2 (now skipped per the plan; promotion is an evidence-class quality concern not numbered-bias work) is what would replace them with outcome-supervised labels once labelled DustEvent rows exist. Risks retired: rules-only attribution lock-in.

**Phase R.2 — Intervention calibration + AP-42 promotion (2026-05-03).**
Status: complete (honest-deferral mode per stop-gate SG-3 — 0 ActionOutcome rows in dev DB). New `app/training/intervention_validation.py:validate_intervention_calibration` joins `ActionOutcome × Recommendation × RecommendationApproval` and returns a `CalibrationReport` with status ∈ {`calibrated`, `deferred`, `violated`}. New `app/domain/intervention_promotion.py:maybe_promote_ap42` decides promotion: (a) if calibrated, promote iff `mean_abs_error_pct < 0.30`; (b) if deferred (current real state), promote iff AP-42's canonical `water_road` + `reduce_speed` reductions both fall in `[0.05, 0.65]`; (c) if violated, do not promote. Wired into the FastAPI lifespan hook alongside the GBM auto-promotion. Verified on the dev DB: report=`deferred` (0 rows), sanity-band probe passed, AP-42 promoted to current. New operator entry point `scripts/validate_intervention_calibration.py`. New `docs/intervention-physics.md` documents the binding contract: physics functions, R.1 documented defaults (silt 8%, Komatsu 930E weight 489 t metric, default meteorology), R.2 calibration-vs-promotion decision tree, and the honest deferral until partnership data lands. 4 new in-memory calibration tests (deferred-on-empty, deferred-below-min, calibrated-at-threshold, sanity-band-constants-in-unit-interval). End of Phase R. Risks introduced: AP-42 effectiveness is grounded in published physics but not yet validated against operator-real outcomes — `main_uncertainty` field on every prediction surfaces this. Risks retired: heuristic-only intervention impact is no longer the live model on any process where the lifespan hook ran.

**Phase R.1 — AP-42 physics-informed intervention model (2026-05-03).**
Status: complete. Replaces the heuristic-only `HeuristicInterventionImpact` for the two interventions where the EPA AP-42 §13.2.2 unpaved-haul-road formula applies. New pure-function module `app/domain/ap42_emission.py` exposes `emission_factor_lb_per_vmt(silt, weight, particle)` (the canonical AP-42 formula `E = k * (s/12)^a * (W/3)^b` with frozen particle-size constants), `speed_reduction_factor(before, after)` (AP-42 speed-exponent 0.5), and `watering_decay_curve(minutes_since_watering, ...meteorology)` (empirical exponential decay parameterised by wind/humidity/temperature so high-evaporation conditions shorten the half-life). New model class `AP42InterventionImpact` (`intervention_impact_ap42_v0.1.0`) routes `water_road` → `watering_decay_curve` and `reduce_speed` → `speed_reduction_factor`; all other intervention IDs fall through to heuristic baseline fractions tagged `source: "heuristic"` (Guardrail 11 spirit). Registered alongside the heuristic baseline; not promoted to current — operator decides per `docs/intervention-physics.md` (Phase R.2 doc). 19 new unit tests covering published reference points (silt=12% + weight=3 short tons → emission factor exactly equals k, the boundary case), PM2.5/PM10 ratio, monotonicity in weight + silt, speed-reduction edge cases, watering decay under arid vs moderate conditions. Risks introduced: physics inputs (silt, vehicle weight, time since watering, meteorology) currently use documented defaults — calibration against real `ActionOutcome` rows is Phase R.2 work. Risks retired: heuristic-only intervention impact for the two AP-42-applicable levers; recommendation engine can now ground its largest intervention claims in EPA physics.

**Phase Q.3 — Multi-station discipline + B-5/B-6/B-14 mitigation (2026-05-03).**
Status: complete. Closes B-5 (survivorship), B-6 (selection), B-14 (distribution shift) with code-level mitigations. New `_multi_station_caveats` in `app/domain/model_performance.py` populates three honest-caveat fields on every `metric_payload` row: `survivor_caveat` (fires when `len(per_receptor) < station_count` or below `MIN_RECEPTORS_FOR_NO_SURVIVOR_CAVEAT=2`); `selection_caveat` (fires when per_receptor spans <2 distinct mines, via a `RECEPTOR_TO_MINE` lookup); `cross_mine_eval` block populated when `compute_metric_payload(trained_on_mine=...)` differs from any receptor's mine. New gate validator `validate-multi-station-discipline.js` (gate #22) asserts (a) `_multi_station_caveats` + `RECEPTOR_TO_MINE` + `MIN_RECEPTORS_FOR_NO_SURVIVOR_CAVEAT` exist, (b) all three caveat keys appear ≥ 2x in the module (both return branches), (c) `compute_metric_payload` accepts `trained_on_mine` kwarg, (d) bias register has flipped B-5/B-6/B-14 to Mitigated (Q.3). Gate count 21 → 22. 11 new pure-function tests in `tests/domain/test_multi_station_discipline.py` covering each caveat's fire/silent paths plus the `compute_metric_payload` integration on both observed and empty branches. `docs/safety-guardrails.md` adds a "Multi-station discipline (Q.3)" section. Risks introduced: receptor-class typing (community vs industrial) deferred — `selection_caveat` uses cross-mine spread as the strongest data-only proxy until that field lands. Risks retired: B-5, B-6, B-14 (now actionable code-level mitigations, not just deferred documentation).

**Phase Q.2 — Shared multi-station GBM (2026-05-03).**
Status: complete. New `app/models/forecasting/gbm_shared_v0_1_0.py` defines `GBMSharedForecaster` — one regressor + one classifier handling all stations via a `station_id` one-hot. New `train_shared_multi_station()` in the training module concatenates all stations' (X, y_pm10, y_breach, ts) tensors, appends per-station one-hot columns, fits one model, persists a single artifact at `data_models/dust_forecast/<model_version>/__shared__/all__<horizon>.joblib`, and writes one M.4 metric_payload row carrying the per_receptor breakdown. New `--shared` CLI flag. First real-DB run: train_n=30,017, test_n=2,709, aggregate ECE 0.003, MAE 11.05 µg/m³, breach_recall 0.09; vocab includes all 5 stations — including Calama Centro which Q.1's per-station fit had skipped (sparse data benefits from the cross-station signal). Distinct `model_version` (`dust_forecast_gbm_shared_v0.1.0`) so registry can resolve both architectures simultaneously; the auto-promotion logic only considers `dust_forecast_gbm_v0.1.0` for now (shared promotion is post-Q.2). Honest limitation surfaced: per-receptor ECE is not currently computed by `_per_receptor_breakdown` in `app/domain/model_performance.py` (only mae_pm10 / breach_precision / recall / fpr / fnr); the planned promotion criterion based on max-per-receptor ECE was therefore deferred — operators can compare via per-receptor MAE instead. 3 new in-memory tests (artifact persistence, per-station one-hot inference, unknown-station warning). Risks introduced: shared model not yet in the auto-promote loop. Risks retired: single-architecture forecast lock-in.

**Phase Q.1 — Multi-station forecast expansion (2026-05-03).**
Status: complete. The Phase P.2 trainer now scales to all 5 SINCA-public stations across 4 mines (Cuncumén / Las Condes / Calama 207 / Calama 275 / Sierra Gorda) under one shared `model_version` (`dust_forecast_gbm_v0.1.0`). Per-station fits live as separate artifacts at `data_models/dust_forecast/<model_version>/<station>__<horizon>.joblib`; `GBMForecaster.predict()` already routed by `(station_id, horizon)` so no model-class changes were needed. New `train_many()` wraps `train_one()` with per-station exception capture so partial outages don't abort the batch (insufficient-data raises a `ValueError` → captured as a placeholder `TrainingResult`; M.4.1 `ProtocolViolation` → re-raises as the SG-1 stop-gate). New `MULTI_STATION_ROSTER` constant; new `STATION_WEATHER_TARGETS` mapping for the 4 newly-paired receptor weather targets (added `weather_targets:` blocks to the 3 new mine YAMLs and pulled 52,704 fresh Open-Meteo records). `scripts/train_dust_forecast.py --all-stations` is the operator entry point. First multi-station run: 4/5 fits + persisted (Calama 275 thin: 66% density, insufficient supervised rows after walk-forward split — captured as placeholder); ECE within gate on every fit, MAE range 1.7-26.1 µg/m³, breach_recall 0.0-0.1 (rare-event class imbalance B-8 expected at receptor stations). 2 new in-memory multi-station tests (happy + placeholder paths). Risks introduced: Calama 275 needs more data before it can promote to current; Sierra Gorda's MAE 26.1 is near the 30 µg/m³ promotion threshold — Q.2 shared model may help. Risks retired: single-station training-pipeline ceiling; M.4.2 per_receptor split is now meaningful with >1 receptor.

**Phase P.3 — First trained forecast model: promotion to current (2026-05-03).**
Status: complete. The trained GBM (`dust_forecast_gbm_v0.1.0`) is now the registry's `current` `dust_forecast` model on any process whose dev DB has a passing metric_payload row. Promotion is data-driven: `app.domain.dust_forecast_promotion.maybe_promote_gbm()` reads `model_performance_metrics.latest_for_version(GBM_VERSION)` and runs `app.training.dust_forecast_training.decide_promotion()` over the persisted M.4 metric_payload. Criteria (conservative defaults): `ece <= protocol.max_ece` (M.4.1 gate, already enforced upstream), `mae_pm10 <= 30 µg/m³`, `observed_count >= 100`. `breach_recall=None` due to rare-event class imbalance does NOT block promotion (an honest property of the test window, not a model failure). Tolerant by design — any exception leaves the heuristic as `current` and logs INFO. Wired into the FastAPI lifespan hook so promotion happens once per process startup; never raises into bootstrap. Layered correctly (api → domain → storage → models registry); models stay pure. 7 new pure-function tests over `decide_promotion` (clean pass, ECE-fail, MAE-fail, thin-sample-fail, MAE-missing, protocol-overrides-default, class-imbalance-pass). Real-DB verification: with the Phase P.2 metric_id=1 row, the GBM is promoted; reverting the row would revert to heuristic. Risks introduced: model is now driving forecasts on dev — operators should check `current_dust_forecast` advertised in `/api/v1/meta` before accepting recommendations. Risks retired: heuristic-only `current` model layer; the M.4 + drift-watch + calibration-badge infrastructure now reflects a real model's behaviour rather than the heuristic's.

**Phase P.2 — First trained forecast model: real fit on Cuncumén (2026-05-03).**
Status: complete. `app.training.dust_forecast_training.train_one` is now the real training pipeline — pulls realtime-PIT PM10 + weather rows, builds the 12-feature matrix declared in P.1, walk-forward fits a `HistGradientBoostingRegressor` (PM10 µg/m³) jointly with a `HistGradientBoostingClassifier` (P(breach @ 150)), synthesizes `TrainingRecordSchema` rows from predictions + actuals, and routes them through `compute_metric_payload(...)` so the M.4.1 ECE acceptance gate, M.4.2 per-receptor split, and Goodhart canary calculation apply uniformly. If the M.4.1 gate raises `ProtocolViolation`, the trainer attempts ONE re-fit with `CalibratedClassifierCV(method="isotonic")`; if that also fails the exception propagates as the SG-1 stop-gate. The fitted artifact is persisted to `data_models/dust_forecast/dust_forecast_gbm_v0.1.0/<station>__<horizon>.joblib` and one M.4 metric_payload row is written. `GBMForecaster.predict()` now lazily loads the artifact and returns a real `DustForecastSchema`. First real run on Cuncumén (12-month window 2025-05-03 → 2026-05-03; 6,451 train / 758 test): ECE ≈ 2e-15, MAE 9.25 µg/m³, breach_recall=None (no breach events in the test window — class imbalance B-8 territory; expected). Note on hindsight discipline: SINCA's col-2 → col-3 promotion delay means the 2025+ window is still pre-validated; the trainer reads pre-validated col-2 PIT records since col-2 is exactly the realtime regime the model would see in production. The legacy `_validated_pm10_series` (col-3-only) was renamed `_realtime_pm10_series` to make the intent explicit. 5 new fast in-memory training tests (synthetic seasonal series; <20s); 1 test fixed to reflect the new artifact-not-found contract instead of the P.1 NotImplementedError. Risks introduced: model is trained on pre-validated values — operator should re-train after the SINCA validation backfill catches up. Risks retired: heuristic-only forecast layer (now displaced for any caller that supplies a fitted artifact, though heuristic remains `current` until P.3 promotion).

**Phase P.1 — First trained forecast model: scaffold (2026-05-03).**
Status: complete. Skeleton-only landing of the first trained `DustForecastModel` implementation. New runtime deps: `numpy`, `scikit-learn`, `joblib` (first ML deps in the repo). New module `app/models/forecasting/gbm_v0_1_0.py` defines `GBMForecaster` implementing the `DustForecastModel` Protocol; `predict()` raises `NotImplementedError` deliberately — the fitted artifact + inference path land in P.2. `app/training/dust_forecast_training.py` declares the canonical Phase P.1 EvaluationProtocol (M.4 shape, walk-forward split, `feature_set` of 12 derivable-from-weather+SINCA features, `max_ece=0.05`, `required_covariates=("humidity_pct",)`, sealed test on the last ~5 weeks of Cuncumén data). Heuristic baseline remains `current` per Guardrail 11; GBM is registered with `set_as_current=False`. New operator entry point `scripts/train_dust_forecast.py` supports `--print-protocol` for pre-registration audit. New `data_models/` directory tracks artifact location but `.gitignore`s the binary blobs (artifacts are reproducible from training data + protocol_hash + code). New `docs/forecast-model-protocol.md` is the binding contract for any trained forecast model. 6 new skeleton tests (model_kind/version, registry hookup, artifact-path resolution, NotImplementedError contract). Risks introduced: first ML deps add ~50MB to the install footprint; mypy slightly slower. Risks retired: heuristic-only forecast layer (now ready to be displaced by a real model).
