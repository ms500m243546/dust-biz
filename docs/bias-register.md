...
| B-5 | Statistical | Survivorship — only 1 of 9 SINCA stations publishes; the survivor may be the "showcase" station | Medium | Q.3 | **Mitigated (Q.3)** | `_multi_station_caveats` populates `survivor_caveat` on every `metric_payload` row whose `len(per_receptor) < station_count` or `< MIN_RECEPTORS_FOR_NO_SURVIVOR_CAVEAT`. |
| B-6 | Statistical | Selection — sensor placement chosen to monitor specific risks; spatial distribution non-random | Medium | Q.3 | **Mitigated (Q.3)** | `_multi_station_caveats` populates `selection_caveat` whenever `per_receptor` spans <2 distinct mines (cross-mine spread is the strongest data-only proxy until receptor classification lands). |
...
| B-14 | Operational | Distribution shift — Los Pelambres → Los Bronces deployment will see different distributions | Medium | Q.3 | **Mitigated (Q.3)** | `_multi_station_caveats` populates `cross_mine_eval` whenever the trained-on mine differs from any evaluated receptor's mine (via `RECEPTOR_TO_MINE`); `compute_metric_payload` accepts `trained_on_mine` kwarg. UI / audit can render the distribution-shift warning on any cross-mine prediction. |
...
| B-8 | Statistical | Class imbalance — breach events are <5% of any test window; vanilla HGBT collapses minority-class gradient and hits 0.0 recall on the noisiest receptors (Las Condes pre-Y). | Medium | Y | **Mitigated (Y)** | `_fit_models` constructs the breach `HistGradientBoostingClassifier` with `class_weight="balanced"`. Calibration gate (M.4.1) still applies; SG-1 isotonic recalibration fallback kicks in if the rebalanced classifier overshoots `max_ece`. |
| B-46 | Operational | Cause–action mismatch — the ranker prefers actions by score alone, ignoring whether the candidate targets the *attributed* cause. A blast-throttle can outrank a haul-road watering even when attribution names the haul road. | Medium | Z | **Mitigated (Z)** | `InterventionOption.target_cause_classes` + ranker boost `w_cause_match` applied only when the active `SourceAttribution` names a cause class in the candidate's targeted list. The boost is gated on actual breach reduction so a do-nothing with a matching cause class can't slip past. Audit: per-candidate `cause_targeted` flag + cause class on `RankedRecommendations`. |
| B-47 | Operational | Plan-blind cost — `tonnes_delayed` is treated identically whether the shift is hours ahead of plan or hours behind. Ranker over-prefers cheap actions during slack windows and under-weighs cost during catch-up windows. | High | AA | **Mitigated (AA)** | Phase AA `compute_shift_progress` + slack-rescale clamps. `w_production` scaled by `1/clamp(slack_ratio, 0.3, 3.0)`; per-candidate `plan_relative_loss ∈ {absorbable, partial, blocking}` exposed for audit. Live mode sums dumping tonnage from `EquipmentActivity` in the shift window; synthetic mode reads operator overrides from `SiteConfiguration.cost_curves`. |
